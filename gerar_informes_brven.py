"""
Gerador de Comprovante de Rendimentos — folha_flavio e folha_beatriz
IN RFB nº 2.060/2021
"""
import os
import pyodbc
...
EMPRESAS = [
    {
        'banco':       'folha_flavio',
        'pasta_saida': r"C:\brven\FLAVIO\informes_pdf",
    },
    {
        'banco':       'folha_beatriz',
        'pasta_saida': r"C:\brven\BEATRIZ\informes_pdf",
    },
]
...
def gerar_pdf(row, pasta_saida):
    ...
    # 5. TRIBUTAÇÃO EXCLUSIVA — 13º líquido = bruto − INSS_13 − IRRF_13
...
def processar_empresa(cfg):
    ...
    print(f"\n{'='*55}")
    print(f"  Banco: {banco}")
    print(f"  PDFs:  {pasta_saida}")
    print(f"{'='*55}")

    os.makedirs(pasta_saida, exist_ok=True)

    dados = buscar_dados(banco)
    if not dados:
